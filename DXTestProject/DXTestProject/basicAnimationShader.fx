// The existing basicShader, but with animation properties.
...
FragmentShaderInput BasicVertexShader( VertexShaderInput input )
{
	...
	float2 newUV;
	newUV.x = (input.texUV.x * frameBounds.z) + frameBounds.x;
	newUV.y = (input.texUV.y * frameBounds.w) + frameBounds.y;
	output.texUV = newUV;
...
float4 BasicFragmentShader( FragmentShaderInput input ) : SV_TARGET
{
	...
	float3 finalColor = (ambientColor + colour * diffuseTerm);
